  ...
  return (
    ...
    <div className="flex h-screen flex-col overflow-hidden bg-[#07070b] text-zinc-100">
      <header className="flex h-14 shrink-0 items-center gap-4 border-b border-white/10 px-5">
        {/* Same badge as the landing: pressing "Start creating" should not feel
            like being handed to a different product. Also the way back. */}
      ...
      {project ? (
        <div className="flex min-h-0 flex-1">
          ...
          <main className="flex min-w-0 flex-1 flex-col gap-4 overflow-y-auto p-5">
            ...
            <details className="group rounded-xl border border-white/10 bg-zinc-900/40">
              ...
              <div className="border-t border-white/10 p-4">{composerBlock}</div>
            </details>

            <section className="rounded-2xl border border-white/10 bg-[#07070b] p-3">
            ...
            {job?.status === "ready" && job.outputUrl ? (
              <section className="overflow-hidden rounded-2xl border border-white/10 bg-black p-2">
            ...
            {job ? (
              <details className="rounded-xl border border-white/10 bg-[#07070b]">
                ...
                <ul className="max-h-56 space-y-0.5 overflow-auto border-t border-white/10 px-4 py-3 font-mono text-[11px] text-zinc-500">
                  {job.logs.map((line, i) => (
                    <li
          ...
          <aside className="flex w-[380px] shrink-0 flex-col gap-4 overflow-y-auto border-l border-white/10 p-4">
            <section className="rounded-2xl border border-white/10 bg-zinc-900 p-4">
                    ...
                    .map((l) => (
                      <li key={l.id} className="flex items-center gap-1">
                        ...
                        <button
                          ...
                          className="shrink-0 rounded-lg border border-white/10 px-2 py-1.5 text-[11px] text-zinc-500 hover:border-red-800 hover:bg-red-950/40 hover:text-red-300"
                        >
                          ✕
                        </button>
                      </li>
      ...
      {/* Slim by design: a workspace should not give a marketing footer more
          than a status bar's worth of height. */}
      <footer className="flex h-9 shrink-0 items-center gap-5 border-t border-white/10 px-5 text-xs text-zinc-600">
        <Link className="transition hover:text-zinc-300" href="/">
          ← Torna alla home
        </Link>
        <Link className="hidden transition hover:text-zinc-300 sm:inline" href="/#faq">
          FAQ
        </Link>
        <span className="ml-auto">© {new Date().getFullYear()} Kinetta</span>
      </footer>
    </div>
  );
}
